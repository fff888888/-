...
from video_search.metadata import VideoMetadata, load_metadata, save_metadata
...
def main() -> None:
    ...
    metadata_inputs, legacy_encoder = _prepare_metadata_paths(
        args.metadata,
        normalized_dir,
        default_dim=args.default_dim,
        encoder_args=args,
    )
    if legacy_encoder is None and args.image_model is not None:
        # 仍旧准备一个 encoder，便于后续 build_or_update_index 在检测到遗留 JSON
        # 缺少 embedding 时可以即时补算（即使上面的转换阶段没有触发）。
        legacy_encoder = _build_legacy_encoder(args)
    build_or_update_index(
        ...
        legacy_feature_root=args.legacy_feature_root,
        legacy_store_inline=bool(not args.no_legacy_export or args.legacy_inline),
    )
...
def _prepare_metadata_paths(
    ...
    *,
...
) -> Tuple[List[Path], Optional[OnnxClipEncoder]]:
    resolved: List[Path] = []
    ...
    for source in paths:
        ...
        store_inline = encoder_args.legacy_inline or target != source

        def _ensure_embeddings(payload: VideoMetadata, base_path: Path) -> None:
            nonlocal encoder
        ...
        if not has_embeddings_source or needs_target_rewrite:
            _ensure_embeddings(metadata, source)
        ...
        base_for_resolution = source if source.exists() else target
        if not metadata_has_embeddings(disk_metadata, base_for_resolution):
            _ensure_embeddings(disk_metadata, base_for_resolution)
        ...
        if not metadata_has_embeddings(metadata, base_for_resolution):
            raise ValueError(
