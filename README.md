...
- **后台处理阶段**：上传完成后立即返回 `job_id`，前端每 1.5 秒轮询 `/api/add_video_status`，实时显示“抽帧 → 生成向量 → 更新索引”等阶段性文案与百分比。
- **完成/失败**：`status=done` 时提示“处理完成，可以开始检索”；`status=error` 时展示后台的报错消息。
...
返回：{"success": bool, "message": str, "job_id": str}

GET /api/add_video_status?job_id=<id>
返回：{"status": "queued|processing|done|error", "stage": "upload|extract_frames|encode|index|done|error", "progress": 0-100, "message": str, "eta_seconds": float|null}

说明：
- `job_id` 现在是自增的简单数字字符串（1、2、3...），方便手动调用 API；若查询时省略 `job_id`，接口会默认返回最近创建的任务状态。
- 上传接口成功返回的 `job_id` 一定会被写入内存队列，状态接口不会再出现“未找到对应任务”的误报。
```
