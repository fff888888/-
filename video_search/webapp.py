      ...
      if (!APP_CONFIG.uploadsEnabled) {
        ...
        const stageName = (stage) => {
          switch (stage) {
            ...
            case 'extracting': return '🔍 正在抽帧与提取特征…';
            case 'indexing': return '📚 正在写入索引…';
            case 'completed': return '✅ 处理完成，可以开始检索';
            case 'error': return '❌ 处理出错，请重新上传';
        ...
        const updateProgressBar = (pct, stageText, etaText) => {
          if (!progressWrapper || !progressContainer) return;
